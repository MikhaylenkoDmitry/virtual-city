...
function cityWall(X1, Y1, R) {
    ...
    function placeWall(x1, y1) {
        ...
        if (x1 > -this.width / 2 && x1 < this.width / 2 && y1 > -this.width / 2 && y1 < this.width / 2 &&
            ...
            if ((img_data.data[xyindex(x1, y1, this.width)] > 0 || img_data.data[xyindex(x2, y2, this.width)] > 0)) {} else
    ...
    for (var j = 0; j < 4; ++j) {
        ...
        while (y >= 0) {
            ...
            if (frst > 50) {
                switch (j) {
                case (0):
                    placeWall(X1 - x * mltpl, Y1 + y * mltpl);
                    break;
                case (1):
                    placeWall(X1 - y * mltpl, Y1 - x * mltpl);
                    break;
                case (2):
                    placeWall(X1 + x * mltpl, Y1 - y * mltpl);
                    break;
...
function placeStuff(scene) {
    ...
    giveMeTheCastle(scene, zm_x_0, 0, zm_y_0, zm_size / 3.5, zm_size / 3.5 * 2, zm_size / 3.5, zm_angle);
